...
This is a commandline line tool that help you to work with AI libraries to
support development.

It is build in rust.

> Note: The agent setup (e.g copilot, codex) need be done before run the cli. It
> means, that you need to open, do login, setup etc. Or write a custom
> agent_command that setup it.
...
To global settings, define it on `$HOME/.quatirc`. For local settings create
`.quatirc` in the root of the git project (same level of `.git` folder).
...
The default behavior for the command `save` is described bellow. The only diff
from save to `update` is that update push the changes to origin.

1. User want to save the changes in a new commit.
2. User run: `quati save`
3. The app saves the the `git diff` result into a temporary file:
   `temp_quati_git_diff.patch`
4. Then the prompt is sent to the agent using the default prompt and default
   command, e.g `codex "$PROMPT"` or `copilot -p $PROMPT`
5. Getting the response message, the app will parse the response, removing any
   other content out of the commit message. It uses the conventional commit
   pattern to understand where the message starts.
...
Mermaid code (compiled in [mermaid-ascii.art](https://mermaid-ascii.art/)):
...
quati ->> git: git diff
git ->> quati: return value
quati ->> quati: create file
quati ->> agent: prompt + diff file
agent ->> quati: return answer
quati ->> quati: format answer
quati ->> git: git add .; git commit -m "{message}"
```
